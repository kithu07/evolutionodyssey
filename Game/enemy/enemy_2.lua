...
function Enemy_2.new(x,y)
   local instance = setmetatable({}, Enemy_2)
   instance.x = x
   instance.y = y
   instance.r = 0
   instance.speed = 50
   ...
   instance.animation.crawl = {total = 4, current = 1, img = Enemy_2.crawlAnim}
...
function Enemy_2.loadAssets()
    Enemy_2.crawlAnim = {}
    for i=1,4 do
        Enemy_2.crawlAnim[i] = love.graphics.newImage("Game/sprites/enemy/crawl/"..i..".png")
    end

    Enemy_2.width = Enemy_2.crawlAnim[1]:getWidth()
    Enemy_2.height = Enemy_2.crawlAnim[1]:getHeight()
...
function Enemy_2:flipDirection()
   self.facingRight = not self.facingRight
   self.xVel = -self.xVel
 end

function Enemy_2:animate(dt)
...
function Enemy_2.updateAll(dt)
   for i,instance in ipairs(ActiveEnemies) do
      instance:update(dt)
   end
end

function Enemy_2.drawAll()
   for i,instance in ipairs(ActiveEnemies) do
      instance:draw()
   end
end

function Enemy_2.beginContact(a, b, collision)
...
function Enemy_2:changeDirection(dt)
